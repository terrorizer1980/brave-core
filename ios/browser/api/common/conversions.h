...
template <class U>
struct remove_pointer<U* const> {
  typedef typename remove_pointer<U>::type type;
};

template <class U>
struct remove_pointer<U* volatile> {
  typedef typename remove_pointer<U>::type type;
};

template <class U>
struct remove_pointer<U* const volatile> {
  typedef typename remove_pointer<U>::type type;
};
...
struct is_objc_convertible {
 public:
  static const bool value =
      std::is_base_of<typename remove_pointer<T>::type,
                      typename remove_pointer<U>::type>::value ||
      std::is_convertible<typename remove_pointer<U>::type,
                          typename remove_pointer<T>::type*>::value;
};
