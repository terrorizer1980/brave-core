...
SegmentList GetTopParentChildInterestSegments(const UserModelInfo& user_model) {
  return GetTopSegments(user_model.interest_segments,
                        kTopInterestSegmentsCount, /* parent_only */ false);
}

SegmentList GetTopParentInterestSegments(const UserModelInfo& user_model) {
    return GetTopSegments(user_model.interest_segments,
                        kTopInterestSegmentsCount, /* parent_only */ true);
}

SegmentList GetTopParentChildLatentInterestSegments(const UserModelInfo& user_model) {
  return GetTopSegments(user_model.latent_interest_segments,
                        kTopLatentInterestSegmentsCount, /* parent_only */ false);
}

SegmentList GetTopParentLatentInterestSegments(const UserModelInfo& user_model) {
    return GetTopSegments(user_model.latent_interest_segments,
                        kTopLatentInterestSegmentsCount, /* parent_only */ true);
}

SegmentList GetTopParentChildPurchaseIntenSegments(const UserModelInfo& user_model) {
    return GetTopSegments(user_model.purchase_intent_segments,
                        kTopPurchaseIntentSegmentsCount, /* parent_only */ false);
}

SegmentList GetTopParentPurchaseIntenSegments(const UserModelInfo& user_model) {
    return GetTopSegments(user_model.purchase_intent_segments,
                        kTopPurchaseIntentSegmentsCount, /* parent_only */ true);
}
