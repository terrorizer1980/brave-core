...
SQLReader::SQLReader(SQLReader&& other) = default;

SQLReader& SQLReader::operator=(SQLReader&& other) = default;
...
Future<SQLReader> SQLStore::RunTransaction(
    std::vector<mojom::DBCommandPtr> commands) {
  auto transaction = mojom::DBTransaction::New();
  transaction->commands = std::move(commands);
  return RunTransactionImpl(std::move(transaction));
...
Future<SQLReader> SQLStore::RunTransactionImpl(
  ...
  DCHECK(transaction);
  FuturePair<SQLReader> future_pair;
  ...
  context().GetLedgerClient()->RunDBTransaction(
      ...
      [resolver](mojom::DBCommandResponsePtr response) mutable {
        ...
        resolver.Complete(SQLReader(std::move(response)));
      });
